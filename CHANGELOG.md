...
- fix: ignore logQuery when Debugger not initiated (in the context of a GitHub Action)

### Security

- harden using `$this->superglobals->get[$this->mapping['code']]`
...
- fix: move seablast-bridge.js above `<header/>` in `BlueprintWeb.latte`, so that ErrorLogger, BannerManager, ButtonPanel and Overlay are initialized prior any code that might be included by an app
...
feat: show database Tracy BarPanel for any Throwable
